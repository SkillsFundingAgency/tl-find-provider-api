...
function FindProvider(
    ...
    console.log('initializing find provider');
    console.log('search term on load = ' + $("#tl-search-term").val());
    ...
    if (searchTerm) {
        ...
        console.log('calling fill form');
        fillFormFromLocalStorage();
    //    console.log('calling clearCheckboxes()');
    //    //checkChange();
    //    clearCheckboxes();
    }
    ...
    function populateRoutes(data) {
                ...
                $.each(item.qualifications,
                    function (_, qualification) {
                        ...
                        skillArea += '<div class="govuk-checkboxes govuk-checkboxes--small"> \
                            ...
                            '" class="tl-checkbox govuk-checkboxes__input" autocomplete="off"> \
        ...
        console.log('calling addCheckboxHandlers');
        addCheckboxHandlers();

        //Logging only
        console.log('added checkbox handlers - current values: ');
        $('#tl-skill-area-filter .tl-checkbox:checked').each(
            function (_, item) {
                console.log('checked box: ' + item.value);
            });
        console.log('added checkbox handlers - all values: ');
        $('#tl-skill-area-filter .tl-checkbox').each(
            function (_, item) {
                console.log('just a box: ' + item.value);
            });    };

    function providerSearch(searchTerm, qualificationIds, page) {
        clearProviderSearchResults();
        searchTerm = searchTerm ? searchTerm.trim() : "";
        console.log('providerSearch - searchTerm is ' + searchTerm);
        if (searchTerm === "") {
    ...
    function showSearchResultsInfoPanel(searchResults) {
        ...
        if (qualificationNames.length === 0 && overMinDistance) {
            ...
            $(".tl-fap--info-panel--detail").append(
                ...
                '<a class="govuk-link tl-fap--no-course-contact" href="https://test.employers.tlevels.gov.uk/hc/en-gb/requests/new">contact us</a> ' +
            ...
            $(".tl-fap--info-panel--detail").append(
                ...
                '<a class="govuk-link tl-fap--no-course-contact" href="https://test.employers.tlevels.gov.uk/hc/en-gb/requests/new">contact us</a>.</p>');
        }
        else if (qualificationNames.length > 1) {
    ...
    function fillFormFromLocalStorage() {
        console.log('fillFormFromLocalStorage');
        if (typeof Storage !== 'undefined') {
            console.log('have local storage');
            const postcode = localStorage.postcode_field;
            console.log('retrieved ' + postcode);
        }
    }
    ...
    function saveFormToLocalStorage() {
        ...
        if (typeof Storage !== 'undefined') {
            console.log('have local storage');
            const postcode = $("#tl-search-term").val().trim(); 
            console.log('saving ' + postcode);
            localStorage.postcode_field = postcode; 
        }
    }

    //var e = document.getElementById("needs-refresh");
    //var fresher = $("#needs-refresh");
    if ($("#needs-refresh").val() === "yes") {
        console.log("needs refresh");
        alert("needs refresh");
    } else {
        console.log("don't need no refresh");
    }
    $("#needs-refresh").val("yes");
};
