CREATE PROCEDURE [dbo].[GetPendingNotificationsWithUpdate]
	@frequency [INT]
AS
	SET NOCOUNT ON;

	CREATE TABLE #TEMP_NOTIFICATIONS (
		[NotificationLocationId] INT PRIMARY KEY, 
		Email NVARCHAR(320)
	);

	DECLARE @metersPerMile DECIMAL = 1609.3399999999999E0;
	DECLARE @notificationDate DATETIME2 = GETUTCDATE();
	...
	WITH Notifications_CTE AS (
			...
			WHERE nl.[Frequency] = @frequency
			  ...
			  AND (nl.[LastNotificationDate] IS NULL
			       OR nl.[LastNotificationDate] < @notificationDate)
			),
	...
	UPDATE  [NotificationLocation]
	SET		[LastNotificationDate] = @notificationDate,
			[ModifiedOn] = GETUTCDATE()
	WHERE [Id] IN (SELECT [Id] FROM #TEMP_NOTIFICATIONS)
