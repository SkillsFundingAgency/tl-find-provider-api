--From https://raw.githubusercontent.com/quartznet/quartznet/main/database/tables/tables_sqlServer.sql
-- This script is for SQL Server and Azure SQL
--
--Modified by adding a GOTO if tables exist
--and commenting out intervening GO statements,
--to work around inability to control flow in SQLCMD
IF EXISTS(select * from sys.tables where name like 'QRTZ_%')
BEGIN
    PRINT 'Skipping Quartz Table creation because tables already exist.'
    GOTO SKIPPED_CREATION
END
...
SKIPPED_CREATION:
--Will skip to here if tables already exist
